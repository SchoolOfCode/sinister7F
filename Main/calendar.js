...
document.addEventListener("click", function (e) {
  if (
    ...
    e.target.classList.contains("active") === null &&
    e.target.classList.contains("day")
  ) {
    e.target.classList.add("active");
  }
...
let newEvent = {
  ...
  submit: () => {
    if (newEvent.desc.value.length === 0) {
      ...
      newEventJson(
        newEvent.desc.value,
        newEvent.month.innerHTML,
        newEvent.year.innerHTML,
        newEvent.active[0].innerHTML
      );
      hideShowEventsDiv();
    ...
    if (eventData.events.length === 0) {
      ...
      [...eventData["events"]].forEach((event) => {
        if (
          ...
          !checkEvents("year", headerYears.innerHTML) ||
...
document.addEventListener("click", (x) => {
  ...
  if (x.target.classList.contains("remove")) {
    ...
    for (var i = eventData.events.length - 1; i >= 0; --i) {
      if (
        eventData.events[i]["day"] === day.innerHTML &&
